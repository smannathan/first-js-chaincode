...
class SmartContractUtil {
    ...
    static async getConnectionProfile() {
        const connectionProfilePath = 'C:\\Users\\smannathan\\.fabric-vscode\\gateways\\local_fabric\\local_fabric.json';
    ...
    static async submitTransaction(contractName, functionName, args, gateway) {
        ...
        let contract;
        if (contractName !== '') {
            contract = await network.getContract('first-js-chaincode', contractName);
        } else {
            contract = await network.getContract('first-js-chaincode');
        }
